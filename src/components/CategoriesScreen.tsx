...
import { Category, ModalConfig } from '../types';
import { get, ref, push, update, remove, rtdb } from '../lib/firebase';
import { formatFriendlyError } from '../lib/errorHandler';
import { checkCategoryDeleteIntegrity } from '../lib/referentialIntegrity';
import { Tag, Plus, Edit2, Trash2, Download, Upload, X } from 'lucide-react';

interface CategoriesScreenProps {
  setModal: (config: ModalConfig) => void;
}

export const CategoriesScreen: React.FC<CategoriesScreenProps> = ({ setModal }) => {
  ...
  const loadCategories = async () => {
    try {
      setLoading(true);
      const snap = await get(ref(rtdb, 'diario-classe/categorias'));
      const val = snap.val() || {};
      const list: { id: string; val: Category }[] = [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await update(ref(rtdb, `diario-classe/categorias/${editingId}`), data);
        setModal({
          isOpen: true,
          type: 'alert',
          title: 'Sucesso',
        ...
        await push(ref(rtdb, 'diario-classe/categorias'), data);
        setModal({
          isOpen: true,
          type: 'alert',
          title: 'Sucesso',
      ...
      setModal({
        ...
        message: formatFriendlyError(err, 'Não foi possível salvar a categoria'),
        icon: '❌'
      });
    }
  };

  const handleEdit = (item: { id: string; val: Category }) => {
    setName(item.val.name);
    setColor(item.val.color || '#667eea');
  ...
  const handleDelete = async (item: { id: string; val: Category }) => {
    // Validação de integridade referencial
    const check = await checkCategoryDeleteIntegrity(item.id, item.val.name);
    if (!check.canDelete) {
      return setModal({
        isOpen: true,
        type: 'alert',
        title: 'Exclusão Bloqueada por Integridade',
        message: check.reason || 'Esta categoria está vinculada a planos de aula e não pode ser excluída.',
        icon: '🛡️'
      });
    }
    ...
    setModal({
      ...
      message: `Tem certeza que deseja excluir a categoria "${item.val.name}"?`,
      icon: '⚠️',
      danger: true,
      onConfirm: async () => {
        try {
          await remove(ref(rtdb, `diario-classe/categorias/${item.id}`));
          setModal({
            isOpen: true,
            type: 'alert',
            title: 'Excluído',
          ...
          setModal({
            ...
            message: formatFriendlyError(err, 'Não foi possível excluir a categoria'),
            icon: '❌'
          });
        }
      }
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6">
      <div className="flex items-center justify-between gap-4 mb-6 flex-wrap">
        <div>
          ...
          <p className="text-xs sm:text-sm text-slate-500">
            Organize os planos de aula por matéria, projetos ou áreas de conhecimento
          </p>
        </div>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div className="sm:col-span-2">
              ...
              <input
                ...
                placeholder="Ex: Língua Portuguesa, Matemática, Artes, Projetos"
      ...
      <div className="bg-white rounded-2xl border border-slate-200/80 p-5 sm:p-6 shadow-xs">
        ...
        {loading ? (
          <div className="py-8 text-center text-sm text-slate-400">Carregando categorias...</div>
        ) : categories.length === 0 ? (
              ...
              >
                <div className="flex items-center gap-2.5">
                  <span
                    ...
                    style={{ backgroundColor: item.val.color || '#667eea' }}
                  ></span>
